...
async def generate_images_from_prompt_list(
    ctx, prompts: list, prefix: str, description: str
):
...
async def generate_image_command(ctx, *, prompt: str):
    ...
    try:
        image = await asyncio.to_thread(
            ctx.bot.hf_client.generate_image, prompt_droplet
        )
        logger.info(
            "⏱ Image generated in %.2f sec | Prompt: %s",
            time.time() - start,
            prompt_droplet,
        )
        buf = io.BytesIO()
...
async def combine_with_lido(ctx, *, user_input: str):
    ...
    await generate_images_from_prompt_list(
        ctx, prompts, "lido", "Testing your input combined with Lido aesthetics"
    )
...
async def combine_with_big_droplet(ctx, *, user_input: str):
    ...
    await generate_images_from_prompt_list(
        ctx, prompts, "big_droplet", "Combining your input with BIG droplet aesthetics"
    )
...
async def combine_with_tech_droplets(ctx, *, user_input: str):
    ...
    await generate_images_from_prompt_list(
        ...
        prompts,
        "tech_droplet",
        "Combining your input with TECH droplet aesthetics",
    )
...
async def combine_with_abstract_droplets(ctx, *, user_input: str):
    ...
    await generate_images_from_prompt_list(
        ...
        prompts,
        "abstract_droplet",
        "Combining your input with ABSTRACT droplet aesthetics",
    )
...
async def combine_inside_droplet(ctx, *, user_input: str):
    ...
    await generate_images_from_prompt_list(
        ctx, prompts, "inside_droplet", "Generating scenes INSIDE a droplet"
    )
